...
model_lidar1 <- sdm(occurrence~.,data=data_forsdm_lidar1,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_sentinel1 <- sdmData(formula=occurrence~., train=train1_sampl[, c(16:22,23)],test=test1_sampl[, c(16:22,23)])
model_sentinel1 <- sdm(occurrence~.,data=data_forsdm_sentinel1,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_landc1 <- sdmData(formula=occurrence~., train=train1_sampl[, c(1:6,23)],test=test1_sampl[, c(1:6,23)])
model_landc1 <- sdm(occurrence~.,data=data_forsdm_landc1,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidsent1 <- sdmData(formula=occurrence~., train=train1_sampl[, c(7:22,23)],test=test1_sampl[, c(7:22,23)])
model_lidsent1 <- sdm(occurrence~.,data=data_forsdm_lidsent1,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidall1 <- sdmData(formula=occurrence~., train=train1_sampl[, c(1:22,23)],test=test1_sampl[, c(1:22,23)])
model_lidall1 <- sdm(occurrence~.,data=data_forsdm_lidall1,methods=c('maxent'),replication=c('boot'),n=20)
...
model_lidar2 <- sdm(occurrence~.,data=data_forsdm_lidar2,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_sentinel2 <- sdmData(formula=occurrence~., train=train2_sampl[, c(16:22,23)],test=test2_sampl[, c(16:22,23)])
model_sentinel2 <- sdm(occurrence~.,data=data_forsdm_sentinel2,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_landc2 <- sdmData(formula=occurrence~., train=train2_sampl[, c(1:6,23)],test=test2_sampl[, c(1:6,23)])
model_landc2 <- sdm(occurrence~.,data=data_forsdm_landc2,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidsent2 <- sdmData(formula=occurrence~., train=train2_sampl[, c(7:22,23)],test=test2_sampl[, c(7:22,23)])
model_lidsent2 <- sdm(occurrence~.,data=data_forsdm_lidsent2,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidall2 <- sdmData(formula=occurrence~., train=train2_sampl[, c(1:22,23)],test=test2_sampl[, c(1:22,23)])
model_lidall2 <- sdm(occurrence~.,data=data_forsdm_lidall2,methods=c('maxent'),replication=c('boot'),n=20)
...
model_lidar3 <- sdm(occurrence~.,data=data_forsdm_lidar3,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_sentinel3 <- sdmData(formula=occurrence~., train=train3_sampl[, c(16:22,23)],test=test3_sampl[, c(16:22,23)])
model_sentinel3 <- sdm(occurrence~.,data=data_forsdm_sentinel3,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_landc3 <- sdmData(formula=occurrence~., train=train3_sampl[, c(1:6,23)],test=test3_sampl[, c(1:6,23)])
model_landc3 <- sdm(occurrence~.,data=data_forsdm_landc3,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidsent3 <- sdmData(formula=occurrence~., train=train3_sampl[, c(7:22,23)],test=test3_sampl[, c(7:22,23)])
model_lidsent3 <- sdm(occurrence~.,data=data_forsdm_lidsent3,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidall3 <- sdmData(formula=occurrence~., train=train3_sampl[, c(1:22,23)],test=test3_sampl[, c(1:22,23)])
model_lidall3 <- sdm(occurrence~.,data=data_forsdm_lidall3,methods=c('maxent'),replication=c('boot'),n=20)
...
model_lidar4 <- sdm(occurrence~.,data=data_forsdm_lidar4,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_sentinel4 <- sdmData(formula=occurrence~., train=train4_sampl[, c(16:22,23)],test=test4_sampl[, c(16:22,23)])
model_sentinel4 <- sdm(occurrence~.,data=data_forsdm_sentinel4,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_landc4 <- sdmData(formula=occurrence~., train=train4_sampl[, c(1:6,23)],test=test4_sampl[, c(1:6,23)])
model_landc4 <- sdm(occurrence~.,data=data_forsdm_landc4,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidsent4 <- sdmData(formula=occurrence~., train=train4_sampl[, c(7:22,23)],test=test4_sampl[, c(7:22,23)])
model_lidsent4 <- sdm(occurrence~.,data=data_forsdm_lidsent4,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidall4 <- sdmData(formula=occurrence~., train=train4_sampl[, c(1:22,23)],test=test4_sampl[, c(1:22,23)])
model_lidall4 <- sdm(occurrence~.,data=data_forsdm_lidall4,methods=c('maxent'),replication=c('boot'),n=20)
...
model_lidar5 <- sdm(occurrence~.,data=data_forsdm_lidar5,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_sentinel5 <- sdmData(formula=occurrence~., train=train5_sampl[, c(16:22,23)],test=test5_sampl[, c(16:22,23)])
model_sentinel5 <- sdm(occurrence~.,data=data_forsdm_sentinel5,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_landc5 <- sdmData(formula=occurrence~., train=train5_sampl[, c(1:6,23)],test=test5_sampl[, c(1:6,23)])
model_landc5 <- sdm(occurrence~.,data=data_forsdm_landc5,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidsent5 <- sdmData(formula=occurrence~., train=train5_sampl[, c(7:22,23)],test=test5_sampl[, c(7:22,23)])
model_lidsent5 <- sdm(occurrence~.,data=data_forsdm_lidsent5,methods=c('maxent'),replication=c('boot'),n=20)

data_forsdm_lidall5 <- sdmData(formula=occurrence~., train=train5_sampl[, c(1:22,23)],test=test5_sampl[, c(1:22,23)])
model_lidall5 <- sdm(occurrence~.,data=data_forsdm_lidall5,methods=c('maxent'),replication=c('boot'),n=20)
