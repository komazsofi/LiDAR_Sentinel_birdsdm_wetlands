...
resp_GrW_sel=getResponseCurve(m_merged_GrW,id = c(1:300))
...
resp_Sn_sel=getResponseCurve(m_merged_Sn,id = c(1:300))
...
j=ggplot(sel_fea1_Sn4,aes(x=lidar_VV_p25,y=meanresp))+geom_line(color="orange",size=2)+
  geom_ribbon(aes(y = meanresp, ymin = meanresp - sdresp/2, ymax = meanresp + sdresp/2),color="orange", alpha = .2)+
  theme_bw(base_size = 30)+ylab("Probability")+xlab("lidar_VV_p25")+ylim(0,1)+ggtitle("(k)")
...
ggsave("fig3_ensav_vxxx_part2.png",plot = fig3b,width = 32, height =15)
